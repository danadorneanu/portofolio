...
int readlines(char *lineptr[], int nlines, int flag);
...
/*  ST_tolower - makes all characters in the string to be lower case */
...
int readlines(char *lineptr[], int maxlines, int flag)
{
   int len = 0, nlines = 0;
   char *p, line[MAXLEN] = {};
...
{
   double v1, v2;
   v1 = atof((char * const) s1);
   v2 = atof((char * const) s2);
   if (v1 < v2)
      return 1;
   else if (v1 > v2)
      return -1;
